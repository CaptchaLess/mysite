...
def split_codes(checkcode):
    ...
    for i in range(4):
        ...
        #plt.show()
        [narrowest_code_array,min_width,min_angle,left,right] = get_narrowest(code_array)
        print('min_width:',min_width)
        print('left:',left)
        print('right:',right)

        bg = np.zeros(shape=(20,20), dtype = np.bool_)
        # bg.dtype= "bool_"
        bg[0:20, (20 - min_width)/2-2:20 - (left - (20-min_width)/2)] = narrowest_code_array[0:20, left-2:20]
